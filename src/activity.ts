...
function init() {
  if (activityList.length === 0) {
    const activities = readFileSync(resolve('data', 'export', 'volunteer.json'), 'utf-8')
...
function appendMemberIntoActivity(
  activities: (ActivityInstance & { oid: number })[],
    ...
    if (idx) {
      ...
      if (activity.type === 'special')
        activity.members.push(
          transformActivityMember(member, mode, member.reward / 60, image) as ActivityMember
        )
      else if (
...
export function transformActivityToJSON() {
  ...
  const members = readFileSync(resolve('data', 'export', 'user_vol.json'), 'utf-8')
  const user_parsed = JSON.parse(members) as V3UserVolunteer[]
  const images = readFileSync(resolve('data', 'export', 'picture.json'), 'utf-8')
  const image_parsed = JSON.parse(images) as V3Picture[]
  const classes = readFileSync(resolve('data', 'export', 'class_vol.json'), 'utf-8')
